...
async function openPopover(type) {
...
<template>
    ...
    >
        ...
        <div v-if="editor" class="main-toolbar">
            ...
            <div class="toolbar-inner">
                ...
                <div class="group">
                    ...
                    <button @click="editor.chain().focus().unsetAllMarks().clearNodes().run()" title="Clear Formatting">
                ...
                <div class="group">
                    ...
                    <div class="size-display">{{ (editor.getAttributes('textStyle').fontSize || '16px').replace('px', '') }}</div>
                ...
                <div class="group">
                    <button @click="editor.chain().focus().toggleSubscript().run()" :class="{ 'active': editor.isActive('subscript') }" title="Subscript">
                         <font-awesome-icon icon="subscript" />
                    </button>
                    <button @click="editor.chain().focus().toggleSuperscript().run()" :class="{ 'active': editor.isActive('superscript') }" title="Superscript">
                ...
                <div class="group">
                    ...
                    <button @click="editor.chain().focus().setTextAlign('justify').run()" :class="{ 'active': editor.isActive({ textAlign: 'justify' }) }" title="Align Justify">
                ...
                <div class="group">
                    ...
                    <button @click="editor.chain().focus().addColumnAfter().run()" :disabled="!editor.isActive('table')" :class="{ 'opacity-20 pointer-events-none': !editor.isActive('table') }" title="Add Column"><font-awesome-icon icon="file-export" class="rotate-90" /></button>
                    <button @click="editor.chain().focus().addRowAfter().run()" :disabled="!editor.isActive('table')" :class="{ 'opacity-20 pointer-events-none': !editor.isActive('table') }" title="Add Row"><font-awesome-icon icon="file-export" /></button>
                    <button @click="editor.chain().focus().deleteTable().run()" :disabled="!editor.isActive('table')" :class="{ 'opacity-20 pointer-events-none': !editor.isActive('table'), 'hover:!text-red-500': editor.isActive('table') }" title="Delete Table"><font-awesome-icon icon="trash-alt" /></button>
                </div>
                ...
                <div class="group">
                    <button @click="openPopover('link')" :class="{ 'active': editor.isActive('link') || activePopover === 'link' }" title="Link">
                        <font-awesome-icon icon="link" />
                    </button>
                    <button @click="openPopover('image')" :class="{ 'active': activePopover === 'image' }" title="Image">
                        <font-awesome-icon icon="image" />
                    </button>
                    <button @click="openPopover('youtube')" :class="{ 'active': activePopover === 'youtube' }" title="YouTube Video">
                ...
                <div class="group">
                    <button class="color-picker-btn">
                        <font-awesome-icon icon="palette" :style="{ color: editor.getAttributes('textStyle').color || 'inherit' }" />
                    ...
                    <button @click="editor.chain().focus().toggleHighlight({ color: highlightColor }).run()" :class="{ 'active': editor.isActive('highlight') }" title="Highlight">
        ...
        <!-- Premium Popover System -->
        <transition name="popover">
            <div v-if="activePopover" class="popover-overlay">
                <div class="popover-card">
                    <div class="popover-header">
                        <span class="capitalize font-bold text-slate-700">{{ activePopover }} URL</span>
                        <button @click="closePopover" class="close-popover">×</button>
                    </div>
                    <div class="popover-body">
                        <input 
                            ...
                            type="text" 
                            :placeholder="'Enter ' + activePopover + ' URL...'" 
                            @keyup.enter="confirmPopover"
                            @keyup.esc="closePopover"
                        />
                        <div class="popover-actions">
                            <button @click="closePopover" class="btn-cancel">Cancel</button>
                            <button @click="confirmPopover" class="btn-confirm" :style="{ background: accentColor }">Apply</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <FloatingMenu v-if="editor" :editor="editor" :tippy-options="{ duration: 100 }" class="floating-menu">
            <button @click="editor.chain().focus().toggleHeading({ level: 1 }).run()" title="Heading 1">H1</button>
            <button @click="editor.chain().focus().toggleHeading({ level: 2 }).run()" title="Heading 2">H2</button>
            <button @click="editor.chain().focus().insertTable({ rows: 3, cols: 3, withHeaderRow: true }).run()" title="Table">
                 <font-awesome-icon icon="table" />
            </button>
            <button @click="openPopover('image')" title="Image">
                <font-awesome-icon icon="image" />
            </button>
        </FloatingMenu>
        ...
        <!-- Stats Footer -->
        <div class="editor-footer" v-if="limit > 0 || wordCount > 0">
            <div class="footer-info">
                <span class="pill">{{ wordCount }} words</span>
                <span class="pill">{{ charCount }}<span v-if="limit > 0"> / {{ limit }}</span> chars</span>
            </div>
            <div v-if="limit > 0" class="progress-wrapper">
